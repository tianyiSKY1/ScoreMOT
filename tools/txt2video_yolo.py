    ...
    for ori_img_path in sorted(ori_img_paths):
        ...
        if img_id in txt_dict:
            for bbox in txt_dict[img_id]:
                ...
                cv2.putText(img, "{:.3f}".format(float(bbox[5])), (int(bbox[0])+5, int(bbox[1])+30), cv2.FONT_HERSHEY_SIMPLEX, 1.0, color_list[bbox[4]%79].tolist(), 1)
...
if __name__ == '__main__':
    ...
    for show_video_name in show_video_names:
        ...
        if args.gt:
            ...
            txt_path = '/home/tianyi/project/ByteTrack/YOLOX_outputs/yolox_x_mot17a/track_results'+ '/' + show_video_name + '.txt'
