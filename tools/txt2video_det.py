...
def txt2img(visual_path="vis_gt",
            ...
            img_path='DanceTrack/val',
            txt_path='DanceTrack/train/dancetrack0004/gt/gt.txt',
            args=None):
    ...
    for ori_img_path in sorted(ori_img_paths):
        ...
        if img_id in txt_dict:
            for bbox in txt_dict[img_id]:
                cv2.rectangle(img, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), color_list[bbox[4]%79].tolist(), thickness=3)
                cv2.putText(img, "{}".format(float(bbox[5])), (int(bbox[0])+5, int(bbox[1])+30), cv2.FONT_HERSHEY_SIMPLEX, 1.0, color_list[bbox[4]%79].tolist(), 2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--det', default=True, action='store_true', help='False is to show prediction')
    parser.add_argument('--tracker', default='transtrack', type=str, help='name of tracker')   
    args = parser.parse_args()
        
    if args.det:
        visual_path = "det" + args.split
    else:
    ...
    for show_video_name in show_video_names:
        if '.ipy' in show_video_name or 'seq' in show_video_name or 'DS_' in show_video_name:
            continue
        if args.det:
            txt_path = '/home/tianyi/project/ByteTrack/datasets/MOT17/' + args.split + '/' + show_video_name + '/det/det.txt'
        else:
            # txt_path = args.split + '/' + args.tracker + '/' + show_video_name + '.txt'
            txt_path = '/home/tianyi/project/ByteTrack/datasets/MOT17/train/' + show_video_name + '.txt'
