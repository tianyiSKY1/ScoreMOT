...
from yolox.tracker_zty import matching_ty as matching
# from yolox.tracker_zty import  matching
from .gmc import GMC
...
class STrack(BaseTrack):
    ...
    def update_features(self, feat):
        ...
        # if (len(self.features) > 10) and (self.frame_id % 20 == 0):
    ...
    def update(self, new_track, frame_id):
        ...
        # # if (len(self.features) > 10) and (self.frame_id % 5 == 0):
        # if (len(self.features) > 10) :
        #     mult_feats = np.array(self.features)
        #     mult_feats = torch.from_numpy(mult_feats).to(torch.device("cuda"))
        #     cluster_ids_x, cluster_centers = kmeans(X=mult_feats, num_clusters=3, distance='cosine',
...
class BoTSORT(object):
    ...
    def update(self, output_results, img):
        ...
        if self.args.with_reid:
            emb_dists = matching.mult_embedding_distance(strack_pool, detections) / 2.0
        ...
        # ious_dists_mask_2 = (ious_dists_2 > self.proximity_thresh)
        # if self.args.with_reid:
        #     # emb_dists_2 = matching.mult_embedding_distance(r_tracked_stracks, detections_second) / 2.0
        #     # raw_emb_dists = emb_dists_2.copy()
        #     # emb_dists_2[emb_dists_2 > self.appearance_thresh] = 1.0
        #     # emb_dists_2[ious_dists_mask_2] = 1.0
        #     # dists_2 = np.minimum(ious_dists_2, emb_dists_2)
        #
        #     # Popular ReID method (JDE / FairMOT)
        #     raw_emb_dists_2 = matching.embedding_distance(r_tracked_stracks, detections_second)
        #     dists_2 = matching.fuse_motion(self.kalman_filter, raw_emb_dists_2, r_tracked_stracks, detections_second)
