...
from .kalman_filter_wh import KalmanFilter
...
class STrack(BaseTrack):
    ...
    def re_activate(self, new_track, frame_id, new_id=False):
        self.mean, self.covariance = self.kalman_filter.update(
            self.mean, self.covariance, self.tlwh_to_xyws(new_track.tlwh)
        )
    ...
    def update(self, new_track, frame_id):
        ...
        self.mean, self.covariance = self.kalman_filter.update(
            self.mean, self.covariance, self.tlwh_to_xyws(new_tlwh))
        self.mean_score, self.covariance_score = self.kalman_filter_score.update(
            self.mean_score, self.covariance_score, self.score)
        self.state = TrackState.Tracked
        self.is_activated = True
...
class BYTETracker(object):
    ...
    def update(self, output_results, img_info, img_size):
        ...
        dists = matching.iou_distance(HighScoreStracks, detections)

        if not self.args.mot20:
            dists = matching.fuse_score(dists, detections)

        # dists = matching.add_score_kalman(dists, strack_pool, detections, 0.6, self.args.track_thresh)
        matches_H, u_track, u_detection = matching.linear_assignment(dists, thresh=0.5)
        ...
        r_tracked_stracks_low = [HighScoreStracks[i] for i in u_track if
                                 HighScoreStracks[i].state == TrackState.Tracked]
        ...
        ious_dists_L = matching.add_score_kalman(ious_dists_L, LowScoreStracks, detections, 0.6, self.args.track_thresh)
        ...
        # dists = matching.add_score_kalman_byte_step(dists, r_tracked_stracks, detections_second, 1.0, self.args.track_thresh)
