...
def add_score(cost_matrix, strack_pool, detections, interval=1.0, track_thresh=0.6):
    if cost_matrix.size == 0:
        return cost_matrix
    strack_score = np.array([np.clip(strack.score_linear, track_thresh, 1.0) for strack in strack_pool])
    ...
    # cost_matrix += (1 / (abs(np.expand_dims(strack_score, axis=1).repeat(cost_matrix.shape[1], axis=1) - det_score)) * interval)
    return cost_matrix

def add_score_byte_step(cost_matrix, strack_pool, detections, interval=1.0, track_thresh=0.6):
    if cost_matrix.size == 0:
        return cost_matrix
    ...
    strack_score = np.array([np.clip(strack.score_linear, 0.1, track_thresh) for strack in strack_pool])
